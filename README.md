...
- **yt-dlp** — optional dependency; auto-installed via pipx on first run if missing (pip as fallback). Install upfront with `pipx install yt-dlp` (or `pacman -S yt-dlp` on Arch).
- **pipx** — required by the runtime auto-installer for yt-dlp on Debian/Ubuntu (`apt install pipx` / `pacman -S pipx`); pip is used as fallback if absent
